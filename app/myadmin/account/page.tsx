...
import { getMember, updateMember, changePasswordMember } from '@/api/member';
...
const AccountPage = () => {
    const { setItem, getItem } = useLocalStorage('tokenAdmin');
    ...
    const handleGetUser = async () => {
      const response = await getMember();
       if (response.code === 'success' && response.data.code == 1) {
         setFullName(response.data.data[0].memberName);
    ...
    const handleChangePassword = async () => {
      ...
      const response = await changePasswordMember({
         memberId: userId,
         password: password,
         email: email,
      });
